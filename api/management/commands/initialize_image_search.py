"""
Django management command to initialize and populate the Qdrant vector database with product images.

Usage:
    python manage.py initialize_image_search
    python manage.py initialize_image_search --local-only
    python manage.py initialize_image_search --database-only
"""
...
def initialize_image_search():
    ...
    try:
        # Import here to avoid locking issues
        from api.image_search_service import initialize_qdrant, get_collection_info
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--reset',
            action='store_true',
            help='Clear existing Qdrant storage before initializing'
        )
            ...
            if options['reset']:
                if os.path.exists(QDRANT_PATH):
                    try:
                        ...
                        self.stdout.write(self.style.SUCCESS(f'✓ Cleared Qdrant storage'))
                        # Wait to ensure the directory is fully removed and locks are released
                        time.sleep(1)
                    except Exception as e:
                        self.stdout.write(self.style.WARNING(f'⚠ Could not fully clear storage: {str(e)}'))
